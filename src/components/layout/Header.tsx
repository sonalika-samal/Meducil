...
export function Header() {
  ...
  useEffect(() => {
    ...
    // Load Google GSI SDK script dynamically
    const loadGsiScript = () => {
          ...
          if (tokenResponse && tokenResponse.access_token) {
            try {
              ...
              if (userData && userData.email) {
                await handleGoogleSelectAccount(userData.name || userData.email.split('@')[0], userData.email);
              }
            } catch (err) {
              console.error("Failed to fetch userinfo from Google:", err);
              setAuthError("Failed to fetch verified user details from Google.");
            }
          }
        },
      });
